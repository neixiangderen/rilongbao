...
// 获取指定元素的某一个 CSS 属性值
// attr : 指定的 CSS 属性名称（字符串）
function css(obj, attr){
	return obj.currentStyle ? obj.currentStyle[attr] : getComputedStyle(obj, null)[attr];
}

// 为指定元素添加指定的事件绑定
// element : 待绑定事件的 DOM 元素
// type : 事件类型
// fn : 事件处理函数
...
function offset(element, coordinates) {
	...
	do {
		...
		if (current){ // 文档定位时，加上盒子边框粗细
			offsetTop += parseFloat(css(current, "borderTopWidth"));
			offsetLeft += parseFloat(css(current, "borderLeftWidth"));
		}
	} while (current !== null);
	...
	if (isGet){ // 获取坐标，则返回表示坐标的对象
		...
		element.style.top = (coordinates.top - offsetTop - parseFloat(css(element.offsetParent, "borderTopWidth"))) + "px";
		element.style.left = (coordinates.left - offsetLeft - parseFloat(css(element.offsetParent, "borderLeftWidth"))) + "px";
	}
}
